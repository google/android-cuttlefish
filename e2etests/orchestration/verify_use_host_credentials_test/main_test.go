...
func TestUseHostCredentials(t *testing.T) {
	...
	_, err := srv.CreateCVD(req, &hoclient.UseHostBuildAPICreds{})

	if err == nil {
		t.Fatal("error: no device should have been created")
	}
	log.Printf("got create error: %s\n", err)
	// cvd pings `metadata.google.internal` aiming to retrieve host's service
	// account credentials as such host is not available `srv.CreateCVD` fails
	// with error mentioning that host: `metadata.google.internal` was not resolved.

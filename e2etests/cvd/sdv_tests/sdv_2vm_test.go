...
func runSdv2VMTest(t *testing.T) {
	...
	t.Run("Status", func(t *testing.T) {
		for _, inst := range []struct {
			...
			entries, err := e2etests.CVDInstanceStatus(&c, inst.name)
			if err != nil {
	...
	t.Run("StopStartInstance2_IsolateInstance1", func(t *testing.T) {
		...
		if err := e2etests.CVDInstanceStop(&c, instance2Name); err != nil {
		...
		if err := e2etests.CVDInstanceStart(&c, instance2Name); err != nil {
	...
	t.Run("RestartInstance2", func(t *testing.T) {
		...
		if err := e2etests.CVDInstanceRestart(&c, instance2Name); err != nil {

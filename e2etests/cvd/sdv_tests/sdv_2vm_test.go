  ...
  "instances": [
    {
      ...
      "boot": {
        "extra_bootconfig_args": "androidboot.sdv.boot_mode=locked androidboot.sdv.ignore_avb_state=true androidboot.sdv.init_open_dice.sample_file=dice_handover_instance1 androidboot.sdv.instance_name=instance1 androidboot.sdv.keymint.rpc.hbk=799da7577efd41d5b27810c5952fcec0291cbcfd687e77ac9a6cec8370651b1d androidboot.sdv.preprovisioned_vvmtruststore=etc androidboot.sdv.someip.enable=true androidboot.sdv.vvmfactorytrust=00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff androidboot.virt.address=3"
      },
      "security": {
    ...
    },
    {
      ...
      "boot": {
        "extra_bootconfig_args": "androidboot.sdv.boot_mode=locked androidboot.sdv.ignore_avb_state=true androidboot.sdv.init_open_dice.sample_file=dice_handover_instance2 androidboot.sdv.instance_name=instance2 androidboot.sdv.keymint.rpc.hbk=a3b876291ff7341d861e325406b96a0b58d0a3daf6b030681b8afc24b84d1160 androidboot.sdv.preprovisioned_vvmtruststore=etc androidboot.sdv.someip.enable=false androidboot.sdv.vvmfactorytrust=00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff androidboot.virt.address=4"
      },
      "security": {
        "guest_enforce_security": false
      },
...
func runSdv2VMTest(t *testing.T) {
	...
	bootMode1, err := c.GetSyspropStringForDevice(instance1Serial, "ro.boot.sdv.boot_mode")
	if err != nil {
		t.Fatalf("failed to read ro.boot.sdv.boot_mode on instance 1 (%s): %v", instance1Serial, err)
	}
	if bootMode1 != expectedBootMode {
		t.Fatalf("unexpected boot mode on instance 1: expected %q, got %q", expectedBootMode, bootMode1)
	}
	...
	bootMode2, err := c.GetSyspropStringForDevice(instance2Serial, "ro.boot.sdv.boot_mode")
	if err != nil {
		t.Fatalf("failed to read ro.boot.sdv.boot_mode on instance 2 (%s): %v", instance2Serial, err)
	}
	if bootMode2 != expectedBootMode {
		t.Fatalf("unexpected boot mode on instance 2: expected %q, got %q", expectedBootMode, bootMode2)
	}
	log.Printf("Verified initial 2-VM state: %s (name=%s, mode=%s), %s (name=%s, mode=%s)",
		instance1Serial, prop1, bootMode1, instance2Serial, prop2, bootMode2)
	...
	t.Run("Status", func(t *testing.T) {
		for _, inst := range []struct {
			...
			entries, err := c.CVDInstanceStatus(inst.name)
			if err != nil {

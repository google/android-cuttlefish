...
// Result<T, E> is the type that is used to pass a success value of type T or an
// error code of type E, optionally together with an error message. T and E can
// be any type. If E is omitted it defaults to int, which is useful when
// errno(3) is used as the error code.
//
...
//
// enum class MyError { A, B }; // assume that this is the error code you
// already have
//
// // To use the error code with Result, define a wrapper class that provides
// the following operations and use the wrapper class as the second type
// parameter (E) when instantiating Result<T, E>
//
...
// ErrnoErrorf("{} errors", num); // equivalent to ErrnoError() << num << "
// errors";
//
...
// Errno is a wrapper class for errno(3). Use this type instead of `int` when
// instantiating `Result<T, E>` and `Error<E>` template classes. This is
// required to distinguish errno from other integer-based error code types like
// `status_t`.
struct Errno {
  Errno() : val_(0) {}
  Errno(int e) : val_(e) {}
  ...
  // TODO(b/209929099): remove this conversion operator. This currently is
  // needed to not break existing places where error().code() is used to
...
namespace internal {
// Stream class that does nothing and is has zero (actually 1) size. It is used
// instead of std::stringstream when include_message is false so that we use
// less on stack. sizeof(std::stringstream) is 280 on arm64.
...
class Error {
 public:
  ...
  operator android::base::expected<T, ResultError<P>>() const {
    return android::base::unexpected(
        ResultError<P>(str(), static_cast<P>(code_)));
  }
  ...
  operator android::base::expected<T, ResultError<P, false>>() const {
    return android::base::unexpected(
        ResultError<P, false>(static_cast<P>(code_)));
  }

  template <typename T>
  Error& operator<<(T&& t) {
    static_assert(include_message,
 ...
 private:
  Error(bool has_code, E code, const std::string& message)
      : code_(code), has_code_(has_code) {
    (*this) << message;
  }

  std::conditional_t<include_message, std::stringstream,
                     internal::DoNothingStream>
      ss_;
  E code_;
  const bool has_code_;
};

inline Error<Errno> ErrnoError() { return Error<Errno>(Errno{errno}); }
...
// Specialization of android::base::OkOrFail<V> for V = Result<T, E>. See
// android-base/errors.h for the contract.
...
// We include the conversion in this base class so that if the conversion in
// NumericConversions overlaps, we (arbitrarily) choose the implementation in
// NumericConversions due to shadowing.
template <typename T>
struct ConversionBase {
  ErrorType<T> error_;
  // T is a expected<U, ErrorType<T>>.
  operator const T() const&& { return unexpected(std::move(error_)); }

  operator const Code<T>() const&& { return error_.code(); }
};
...
// android::base::Result. These also work with base::android::expected. For
// advanced matchers and customized error messages, see result-gtest.h.

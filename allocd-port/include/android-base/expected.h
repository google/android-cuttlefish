...
class _NODISCARD_ expected {
 public:
  ...
  template <
      class G = E _ENABLE_IF(std::is_constructible_v<E, const G&> &&
                             !std::is_convertible_v<const G&, E> /* explicit
                                                                  */
                             )>
  ...
  template <class G = E _ENABLE_IF(std::is_constructible_v<E, G&&>&& std::
                                       is_convertible_v<G&&, E> /* non-explicit
                                                                 */
                                   )>
  // NOLINTNEXTLINE(google-explicit-constructor)
  ...
  template <class G = E _ENABLE_IF(std::is_nothrow_move_constructible_v<G>&&
  ...
  // modifiers
  template <
      class... Args _ENABLE_IF(std::is_nothrow_constructible_v<T, Args...>)>
  T& emplace(Args&&... args) {
  ...
  template <typename U = T,
            typename = std::enable_if_t<(std::is_swappable_v<U> &&
...
class _NODISCARD_ expected<void, E> {
 public:
  ...
  template <class U,
            class G _ENABLE_IF(std::is_void_v<U>&& std::is_convertible_v<
                               const G&, E> /* non-explicit */
                               )>
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr expected(const expected<U, G>& rhs) {
  ...
  friend constexpr bool operator==(const expected<void, E1>& x,
...
class unexpected {
 public:
  ...
  template <class Err = E _ENABLE_IF(
                ...
                !std::is_same_v<std::remove_cv_t<std::remove_reference_t<E>>,
                                std::in_place_t> &&
                !std::is_same_v<std::remove_cv_t<std::remove_reference_t<E>>,
                                unexpected>)>
  // NOLINTNEXTLINE(google-explicit-constructor,bugprone-forwarding-reference-overload)
  ...
  constexpr unexpected& operator=(unexpected&&) noexcept(
      std::is_nothrow_move_assignable_v<E>) = default;
  ...
  template <class E1, class E2>
  friend constexpr bool operator==(const unexpected<E1>& e1,
  ...
  template <class E1>
  friend void swap(unexpected<E1>& x,
                   unexpected<E1>& y) noexcept(noexcept(x.swap(y)));

...
// Convenient macros for evaluating a statement, checking if the result is
// error, and returning it to the caller.
//
// Usage with Result<T>:
//
// Result<Foo> getFoo() {...}
...
// }
//
// Actually this can be used for any type as long as the OkOrFail<T> contract is
// satisfied. See below. If implicit conversion compilation errors occur
// involving a value type with a templated forwarding ref ctor, compilation with
// cpp20 or explicitly converting to the desired return type is required.
#define OR_RETURN(expr)                                                     \
  ({                                                                        \
    decltype(expr)&& tmp = (expr);                                          \
    typedef android::base::OkOrFail<std::remove_reference_t<decltype(tmp)>> \
        ok_or_fail;                                                         \
...
// The OkOrFail contract for a type T. This must be implemented for a type T if
// you want to use OR_RETURN(stmt) where stmt evalues to a value of type T.

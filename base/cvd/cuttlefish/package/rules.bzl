...
def _package_files_impl(ctx):
    ...
    for (dst, target) in ctx.attr.dst_to_target_symlinks.items():
        ...
        if src_file == None:
            fail(msg = "Target \"{}\" in \"dst_to_target_symlinks\" does not exist as a \"dst\" in \"dst_to_src_mapping\"".format(target))
...
package_files = rule(
    attrs = {
        ...
        "dst_to_src_mapping": attr.string_keyed_label_dict(
            doc = "Output file paths to the target providing the file.",
            allow_files = True),
        "dst_to_target_symlinks": attr.string_dict(
            doc = """Output symlink paths to the target path. The target path must exist in "dst_to_src_mapping".""",
        ),
    },
    executable = True,

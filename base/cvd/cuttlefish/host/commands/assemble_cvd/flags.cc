...
Result<CuttlefishConfig> InitializeCuttlefishConfiguration(
  ...
  for (const auto& num : instance_nums) {
    ...
    if (vhost_user_vsock_vec[instance_index] == kVhostUserVsockModeAuto) {
      ...
      } else if (IsCrosvm(tmp_config_obj) &&
    ...
    if (FLAGS_use_overlay && !IsGem5(vm_manager_flag)) {
    ...
    bool persistent_disk = !IsGem5(vm_manager_flag);
    if (persistent_disk) {
#ifdef __APPLE__
...
#else
      const std::string persistent_composite_img_base =
          IsQemu(tmp_config_obj) ? "persistent_composite_overlay.img"
                                 : "persistent_composite.img";
#endif
      auto path =

...
TEST_P(HomeTest, HomeTest) {
  ...
  ASSERT_EQ(result.has_value(), expected_success_) << result.error().Trace();
...
TEST(AutoHomeTest, DefaultFailAtSecondTrialTest) {
  ...
  if (!result_db_addition.has_value()) {
  ...
  ASSERT_TRUE(result_1st_exec.has_value()) << result_1st_exec.error().Trace();
  ASSERT_EQ(result_1st_exec->home, TestUserHome());
  ASSERT_FALSE(result_2nd_exec.has_value())
...
TEST(AutoHomeTest, DefaultFollowedByNonDefaultTest) {
  ...
  ASSERT_TRUE(result_default.has_value()) << result_default.error().Trace();
  ASSERT_EQ(result_default->home, TestUserHome());
  ASSERT_TRUE(result_1st_non_default.has_value())
      << result_1st_non_default.error().Trace();
  ASSERT_EQ(result_1st_non_default->home, AutoGeneratedHome("goog"));
  ASSERT_TRUE(result_2nd_non_default.has_value())

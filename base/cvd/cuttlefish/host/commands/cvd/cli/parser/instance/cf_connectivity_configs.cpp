...
static int32_t ModemSimulatorSimType(const Instance& instance) {
  if (instance.connectivity().has_modem_simulator_sim_type()) {
    return instance.connectivity().modem_simulator_sim_type();
  } else {
...
std::vector<std::string> GenerateConnectivityFlags(
  ...
  const bool has_sim_type = std::any_of(cfg.instances().begin(), cfg.instances().end(),
                                        [](const Instance& ins) {
                                          return ins.connectivity().has_modem_simulator_sim_type();
                                        });
  if (has_sim_type) {
    flags.push_back(GenerateInstanceFlag(kFlagModemSimulatorSimType, cfg, ModemSimulatorSimType));
  }
  return flags;
}

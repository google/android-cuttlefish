...
std::string AssertionClaims(std::string_view body) {
...
class StorageCredentialTests : public ::testing::Test {
 protected:
  ...
  std::string WriteTempFile(const std::string& name,
    ...
    EXPECT_THAT(WriteNewFile(path, contents), IsOk());
    return path;
  }
...
TEST_F(StorageCredentialTests,
       CredentialSourceTokenIsNotPresentedToStorageSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests, CredentialSourceFileIsNeverReadSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests,
       CredentialFilepathIsNotPresentedToStorageSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests, ServiceAccountMintsAStorageScopedTokenSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests, BotoFileOutranksTheMetadataServerSuccess) {
  BuildApiFlags flags;
  WriteTempFile(".boto",
...
TEST_F(StorageCredentialTests, GceHostsGetAnAmbientCredentialSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests,
       UseGceMetadataFlagQueriesTheMetadataServerSuccess) {
  BuildApiFlags flags;
...
TEST_F(StorageCredentialTests, LegacyGceValueQueriesTheMetadataServerSuccess) {
  BuildApiFlags flags;

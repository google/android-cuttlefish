...
static std::string GpuMode(const Instance& instance) {
  const auto& graphics = instance.graphics();
  return graphics.has_gpu_mode() ? graphics.gpu_mode() : /* CF_DEFAULTS_GPU_MODE */ "auto";
}

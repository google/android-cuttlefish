...
class RecordingBuildApi : public BuildApi {
 public:
  ...
  Result<std::string> DownloadFile(const Build&,
    ...
    CF_EXPECT(WriteNewFile(path, kContents));
    return path;
  }
...
TEST_F(CachingBuildApiTests,
       DownloadFileOfAnUnversionedArtifactSkipsTheCacheSuccess) {
...
TEST_F(CachingBuildApiTests,
       DownloadFileOfAnUnlistedArtifactSkipsTheCacheSuccess) {
  GcsBuild build =
      *GcsBuild::FromBuildString(GcsBuildString{.url = "gs://bucket/dist/"});

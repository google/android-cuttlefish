...
Result<std::string> CreateJwt(const std::string& email,
                              const std::string& scope, EVP_PKEY* private_key) {
  ...
  std::unique_ptr<EVP_MD_CTX, void (*)(EVP_MD_CTX*)> sign_ctx(EVP_MD_CTX_new(),
                                                              EVP_MD_CTX_free);
...
RefreshingCredentialSource::RefreshingCredentialSource()
    : expiration_(std::chrono::steady_clock::time_point::min()) {}

Result<std::string> RefreshingCredentialSource::Credential() {

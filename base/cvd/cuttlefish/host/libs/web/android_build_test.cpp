...
TEST(GcsBuildTests, ObjectAtBucketRootSuccess) {
  Result<GcsBuild> build = GcsBuild::FromBuildString(
      GcsBuildString{.url = "gs://bucket/phone-img-1.zip"});
...
TEST(HttpBuildTests, DirectoryFormSuccess) {
  ...
  ASSERT_THAT(build, IsOk());
  EXPECT_EQ(build->url, "https://example.com/dist/");
  EXPECT_EQ(build->object, std::nullopt);

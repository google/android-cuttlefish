...
Result<std::vector<uint8_t>> DigestFile(const std::string& path,
                                        const EVP_MD* digest) {
  Fd fd = CF_EXPECT(Fd::Open(path, O_RDONLY));
  ...
  while (true) {
    const uint64_t read = CF_EXPECTF(fd.Read(buffer.data(), buffer.size()),
                                     "Could not read '{}'", path);
    if (read == 0) {
      break;
    }
...
Result<void> VerifySha256(const std::string& path, std::string_view expected,
  ...
  const std::string actual = CF_EXPECT(Sha256File(path));
...
Result<void> VerifyMd5(const std::string& path, std::string_view expected,
                       std::string_view artifact_name) {
  const std::vector<uint8_t> value = CF_EXPECT(DigestFile(path, EVP_md5()));
  const std::string actual =
      CF_EXPECT(EncodeBase64(value.data(), value.size()));

...
// string, so requests use `url` or `base`.
struct HttpBuild {
...
// Returns empty when the source reports no version, which means the artifact
// must not be cached.
std::string BuildCacheKey(const Build& build, const std::string& artifact_name);

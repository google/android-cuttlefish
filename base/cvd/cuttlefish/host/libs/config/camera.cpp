...
Result<std::optional<CuttlefishConfig::CameraConfig>> ParseCameraConfig(
  ...
  CuttlefishConfig::CameraType type =
      static_cast<CuttlefishConfig::CameraType>(CF_DEFAULTS_CAMERA_TYPE);
  auto type_it = props.find("type");
  if (type_it != props.end()) {
    CF_EXPECTF(type_it->second == kCameraTypeV4l2Emulated ||
                   type_it->second == kCameraTypeV4l2Proxy,
               "Unknown camera type value: \"{}\"", type_it->second);

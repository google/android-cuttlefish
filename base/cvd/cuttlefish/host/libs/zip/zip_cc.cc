...
Result<ReadableZipSource> ReadableZipSource::FromCallbacks(
  ...
  return ReadableZipSource(std::unique_ptr<Impl>(new Impl(std::move(source))));
}
...
Result<SeekableZipSource> SeekableZipSource::FromCallbacks(
  ...
  return SeekableZipSource(std::unique_ptr<Impl>(new Impl(std::move(source))));
}
...
Result<WritableZipSource> WritableZipSource::CopyData(const void* data,
                                                      size_t size) {
  ...
  return WritableZipSource(std::unique_ptr<Impl>(new Impl(std::move(source))));
}
...
Result<ReadableZip> ReadableZip::FromSource(SeekableZipSource source) {
  ...
  return ReadableZip(std::unique_ptr<Impl>(
      new Impl(std::move(zip_ret), std::move(fake_writable_source))));
}
...
Result<SeekableZipSource> ReadableZip::GetFile(uint64_t index) {
  ...
  return SeekableZipSource(std::unique_ptr<ReadableZipSource::Impl>(
      new ReadableZipSource::Impl(std::move(raw_source))));
}

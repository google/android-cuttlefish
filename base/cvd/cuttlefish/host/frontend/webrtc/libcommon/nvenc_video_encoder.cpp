...
std::string GetCudaErrorStr(const CudaFunctions* cuda, CUresult err) {
...
NvencVideoEncoder::NvencVideoEncoder(const NvencEncoderConfig& config,
  ...
  VLOG(1) << "Creating NvencVideoEncoder (" << config_.implementation_name
          << ") for format: " << format.ToString();
}

NvencVideoEncoder::~NvencVideoEncoder() { DestroyEncoder(); }
...
int NvencVideoEncoder::InitEncode(
  ...
  VLOG(1) << "NvencVideoEncoder::InitEncode (" << config_.implementation_name
          << ")";
  ...
  VLOG(1) << "InitEncode parameters: " << width_ << "x" << height_ << " @"
          << framerate_ << "fps, " << bitrate_bps_
          << "bps, max=" << codec_settings->maxBitrate << "kbps";
...
Result<void> NvencVideoEncoder::InitCuda() {
  ...
  CF_EXPECT(shared_context_ != nullptr,
            "Failed to get CUDA context for device " << kGpuDeviceId);
  ...
  err = cuda_->cuMemAllocPitch(&d_rgba_buffer_, &rgba_pitch_, width_ * 4,
                               height_, 4);
  CF_EXPECT(err == CUDA_SUCCESS,
            "cuMemAllocPitch failed: " << GetCudaErrorStr(cuda_, err));

  VLOG(1) << "InitCuda: CUDA resources allocated, pitch=" << rgba_pitch_;
  return {};
}
...
Result<void> NvencVideoEncoder::InitNvenc() {
  ...
  NVENCSTATUS status =
      nvenc_funcs_->nvEncOpenEncodeSessionEx(&open_params, &raw_encoder);
  CF_EXPECT(status == NV_ENC_SUCCESS,
            "nvEncOpenEncodeSessionEx failed: " << status);
  encoder_ = NvencEncoderHandle(raw_encoder, NvencEncoderDeleter{nvenc_funcs_});
  ...
  stored_init_params_.tuningInfo = NV_ENC_TUNING_INFO_ULTRA_LOW_LATENCY;
  ...
  status = nvenc_funcs_->nvEncGetEncodePresetConfigEx(
      ...
      stored_init_params_.presetGUID, stored_init_params_.tuningInfo,
      &preset_config);
  CF_EXPECT(status == NV_ENC_SUCCESS,
  ...
  stored_encode_config_.rcParams.rateControlMode = NV_ENC_PARAMS_RC_CBR;
  ...
  status = nvenc_funcs_->nvEncInitializeEncoder(encoder_.get(),
                                                &stored_init_params_);
  CF_EXPECT(status == NV_ENC_SUCCESS,
  ...
  status = nvenc_funcs_->nvEncCreateBitstreamBuffer(encoder_.get(), &create_bs);
  CF_EXPECT(status == NV_ENC_SUCCESS,
            "nvEncCreateBitstreamBuffer failed: " << status);
  nvenc_output_bitstream_ = create_bs.bitstreamBuffer;

  LOG(INFO) << "NVENC initialized: " << config_.implementation_name << " "
            << width_ << "x" << height_ << " @" << framerate_ << "fps, "
            << bitrate_bps_ << "bps";
  return {};
}

Result<void> NvencVideoEncoder::RegisterInputResource(uint32_t pixel_format) {
  ...
  if (pixel_format == DRM_FORMAT_ARGB8888 ||
    ...
    return CF_ERR("Unsupported pixel format: 0x" << std::hex << pixel_format);
  }
  ...
  reg_res.resourceToRegister = reinterpret_cast<void*>(d_rgba_buffer_);
  ...
  NVENCSTATUS status =
      nvenc_funcs_->nvEncRegisterResource(encoder_.get(), &reg_res);
  CF_EXPECT(status == NV_ENC_SUCCESS,
  ...
  VLOG(1) << "Input resource registered for pixel format 0x" << std::hex
          << pixel_format;
  return {};
}
...
void NvencVideoEncoder::SetRates(const RateControlParameters& parameters) {
  ...
  if (parameters.bitrate.get_sum_bps() != 0) {
    int32_t requested = static_cast<int32_t>(parameters.bitrate.get_sum_bps());
...
void NvencVideoEncoder::Reconfigure(const RateControlParameters& parameters) {
  if (!encoder_) {
  ...
  NVENCSTATUS status =
      nvenc_funcs_->nvEncReconfigureEncoder(encoder_.get(), &reconfig);
  if (status != NV_ENC_SUCCESS) {
    LOG(ERROR) << "nvEncReconfigureEncoder failed: " << status;
  } else {
    VLOG(1) << "NVENC reconfigured: bitrate=" << bitrate_bps_
            << ", framerate=" << stored_init_params_.frameRateNum;
  }
}
...
Result<void> NvencVideoEncoder::EncodeInner(
  ...
  CF_EXPECT(frame.width() == width_ && frame.height() == height_,
            "Resolution changed from " << width_ << "x" << height_ << " to "
                                       << frame.width() << "x"
                                       << frame.height());
  ...
  CUresult cuda_err = cuda_->cuMemcpy2DAsync(&copy_desc, cuda_stream_);
  CF_EXPECT(cuda_err == CUDA_SUCCESS,
            "cuMemcpy2DAsync failed: " << GetCudaErrorStr(cuda_, cuda_err));

  cuda_err = cuda_->cuStreamSynchronize(cuda_stream_);
  CF_EXPECT(cuda_err == CUDA_SUCCESS,
            "cuStreamSynchronize failed: " << GetCudaErrorStr(cuda_, cuda_err));

  // Map input resource
  NV_ENC_MAP_INPUT_RESOURCE map_input = {NV_ENC_MAP_INPUT_RESOURCE_VER};
  map_input.registeredResource = nvenc_input_buffer_;
  NVENCSTATUS status =
      nvenc_funcs_->nvEncMapInputResource(encoder_.get(), &map_input);
  CF_EXPECT(status == NV_ENC_SUCCESS,
  ...
  status = nvenc_funcs_->nvEncEncodePicture(encoder_.get(), &pic_params);
  if (status != NV_ENC_SUCCESS) {
    nvenc_funcs_->nvEncUnmapInputResource(encoder_.get(),
                                          map_input.mappedResource);
  ...
  status = nvenc_funcs_->nvEncLockBitstream(encoder_.get(), &lock_bs);
  CF_EXPECT(status == NV_ENC_SUCCESS, "nvEncLockBitstream failed: " << status);
  ...
  encoded_image._frameType = (lock_bs.pictureType == NV_ENC_PIC_TYPE_IDR ||
                              lock_bs.pictureType == NV_ENC_PIC_TYPE_I)
                                 ? webrtc::VideoFrameType::kVideoFrameKey
                                 : webrtc::VideoFrameType::kVideoFrameDelta;

  nvenc_funcs_->nvEncUnlockBitstream(encoder_.get(), nvenc_output_bitstream_);

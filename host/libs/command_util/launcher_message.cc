...
Result<LauncherActionMessage> LauncherActionMessage::Create(
  ...
  CF_EXPECTF(IsShortAction(action),
             "LauncherAction {} is not supported by legacy "
             "LauncherActionMessage::Create()",
...
Result<LauncherActionMessage> LauncherActionMessage::Create(
  ...
  if (IsShortAction(action)) {
    ...
    CF_EXPECTF(
        ...
        "The type of action \"{}\" should be ExtendedActionType::kUnused",
        action_char);
...
static Result<void> WriteBuffer(const SharedFD& fd, const std::string& buf,
  ...
  CF_EXPECTF(bytes_sent > 0, "Error sending {} to launcher monitor: {}",
             description, fd->StrError());
...
Result<LauncherActionMessage> LauncherActionMessage::ReadFromFd(
  ...
  CF_EXPECTF((n_bytes = fd->Read(&action, sizeof(action))) > 0,
             "The returned n_bytes is not {} but {}: {}", sizeof(action),
             n_bytes, fd->StrError());
  if (IsShortAction(action)) {
  ...
  CF_EXPECTF((n_bytes = fd->Read(&type, sizeof(type))) > 0,
             "The returned n_bytes is not {} but {}", sizeof(type), n_bytes);
  SerializedDataSizeType length = 0;
  CF_EXPECTF((n_bytes = fd->Read(&length, sizeof(length))) > 0,
             "The returned n_bytes is not {} but {}", sizeof(length), n_bytes);
  if (length == 0) {

...
function trackTouchEvents(videoElement, dc) {
  // No need to track state since mouse hovers are not reported

  function onTouchEvent(e) {
    // Can't prevent event default behavior to allow the element gain focus
    // when touched and start capturing keyboard input in the parent.
    const down = e.type == "touchstart" || e.type == "touchmove"
    ...
    sendEventUpdate(dc, e.target, events, down);
  }

  videoElement.addEventListener('touchstart', onTouchEvent);
  videoElement.addEventListener('touchmove', onTouchEvent);
  videoElement.addEventListener('touchend', onTouchEvent);
  videoElement.addEventListener('touchcancel', onTouchEvent);
}

function trackMouseEvents(videoElement, dc) {
  let mouseDown = false;
  function onMouseDown(e) {
    // Can't prevent event default behavior to allow the element gain focus
    // when touched and start capturing keyboard input in the parent.
    mouseDown = true;
    sendEventUpdate(dc, e.target, [{x: e.offsetX, y: e.offsetY, id: 1}], mouseDown);
  }
  ...
  videoElement.addEventListener('mouseleave', onMouseUp);
}

function emulateTouchScreen(videoElement, dc) {
  if (window.PointerEvent) {
    trackPointerEvents(videoElement, dc);
  } else if (window.TouchEvent) {
    trackTouchEvents(videoElement, dc);
  } else if (window.MouseEvent) {
    trackMouseEvents(videoElement, dc);
  }
}

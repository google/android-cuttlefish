...
function ConnectToDevice(device_id) {
  ...
  function showAdbConnected() {
    ...
    statusMessage.textContent = 'adb connection established successfully.';
    setTimeout(function() {
  ...
  function initializeAdb() {
    init_adb(deviceConnection, showAdbConnected, function() {
      statusMessage.className = 'error';
  ...
  function createControlPanelButton(
      command, title, icon_name, listener = onControlPanelButton,
    ...
    buttons[command] = { 'button': button } return buttons[command];
  }
  ...
  createControlPanelButton(
      'rotate', 'Rotate', 'screen_rotation', onRotateButton);
  ...
  let modalOffsets = {} function createModalButton(
      button_id, modal_id, close_id) {
  ...
  function updateDeviceStateDetails() {
    ...
    if (deviceStateHingeAngleValue != null) {
      deviceStateDetailsTextLines.push(
          `Hinge Angle - ${deviceStateHingeAngleValue}`);
    }
  ...
  bluetoothConsole.addCommandListener(cmd => {
    ...
    inputArr.shift();
    let args = inputArr;
    deviceConnection.sendBluetoothMessage(

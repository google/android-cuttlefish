...
export class ViewPaneComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    this.visibleSubscription = this.visibleDevices.subscribe(
      (allDisplays: Device[]) => {
        this.layout = this.layout.filter((ktdGridItem: KtdGridLayoutItem) =>
          allDisplays.some(aDisplay => aDisplay.deviceId === ktdGridItem.id)
        );
        allDisplays
          .filter(
            (aDisplay: Device) =>
              !this.layout.some(
                (ktdGridItem: KtdGridLayoutItem) =>
                  aDisplay.deviceId === ktdGridItem.id
              )
          )
          .forEach((addDisplay: Device) =>
            this.addToLayout(addDisplay.deviceId)
          );
  ...
  onLayoutUpdated(layout: KtdGridLayout) {
    this.layout = layout;
    this.createdDeviceLayout = this.createdDeviceLayout.filter(
      obj => !layout.some(element => element.id === obj.id)
    );
  ...
  addDeviceToLayout(display: string) {
    ...
    const lastMaxY =
      this.layout.length !== 0 ? Math.max(...this.layout.map(obj => obj.y)) : 0;
    ...
    const newLayoutItem: KtdGridLayoutItem = {
      ...
      y:
        lastMaxX + newItemWidth * 2 <= this.cols
          ? lastMaxY
          : lastMaxY + newItemHeight,
      w: newItemWidth,
      h: newItemHeight,
    };

...
type UserArtifactsManager interface {
	...
	UpdateArtifact(chunk UserArtifactChunk) error
...
func (m *UserArtifactsManagerImpl) UpdateArtifact(chunk UserArtifactChunk) error {
	...
	mayMoveArtifact, err := m.writeChunkAndUpdateState(chunk)
	if err != nil {
...
func (m *UserArtifactsManagerImpl) getChunkState(chunk UserArtifactChunk) *ChunkState {
	cs, _ := m.chunkStates.LoadOrStore(chunk.Checksum, NewChunkState(chunk.FileSizeBytes))
...
func (m *UserArtifactsManagerImpl) writeChunkAndUpdateState(chunk UserArtifactChunk) (bool, error) {
	mu := m.getRWMutex(chunk.Checksum)
	...
	mu.RLock()
	defer mu.RUnlock()
	if exists, err := dirExists(filepath.Join(m.RootDir, chunk.Checksum)); err != nil {
		return false, fmt.Errorf("failed to check existence of directory: %w", err)
	} else if exists {
		return false, operator.NewConflictError(fmt.Sprintf("user artifact(checksum:%q) already exists", chunk.Checksum), nil)
	}
	...
	workDir := filepath.Join(m.UuidWorkDir, chunk.Checksum)
...
func (m *UserArtifactsManagerImpl) validateChecksum(chunk UserArtifactChunk) (bool, error) {
	workFile := filepath.Join(m.UuidWorkDir, chunk.Checksum, chunk.Name)
	...
	return chunk.Checksum == fmt.Sprintf("%x", h.Sum(nil)), nil
}

func (m *UserArtifactsManagerImpl) moveArtifactIfNeeds(chunk UserArtifactChunk) error {
	mu := m.getRWMutex(chunk.Checksum)
	// Reason for acquiring write lock is to restrict writing new chunks while validating checksum
	// or moving artifact.
	mu.Lock()
	defer mu.Unlock()
	dir := filepath.Join(m.RootDir, chunk.Checksum)
	if exists, err := dirExists(dir); err != nil {
		return fmt.Errorf("failed to check existence of directory: %w", err)
	} else if exists {
		return nil
	}
	if matches, err := m.validateChecksum(chunk); err != nil {
	...
	m.chunkStates.LoadAndDelete(chunk.Checksum)
	return nil
}

...
func toApiv1ListRecordingsResponse(recordings []string) *apiv1.ScreenRecordingListResponse {
	response := apiv1.ScreenRecordingListResponse{}
	for _, recording := range recordings {
		response = append(response, filepath.Base(recording))
	}
	return &response
}

func (a *ListScreenRecordingsAction) Run() (*apiv1.ScreenRecordingListResponse, error) {

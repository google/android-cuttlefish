...
func (h *createSnapshotHandler) Handle(r *http.Request) (interface{}, error) {
	...
	if req.SnapshotID != "" {
	  if err := uuid.Validate(req.SnapshotID); err != nil {
		return nil, operator.NewBadRequestError(fmt.Sprintf("snapshot id %s is an invalid uuid value", req.SnapshotID), err)
	  }
	}
	vars := mux.Vars(r)
	group := vars["group"]

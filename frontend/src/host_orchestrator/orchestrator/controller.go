...
func (c *Controller) AddRoutes(router *mux.Router) {
	...
	router.Handle("/cvds/{group}/{name}/event_devices",
		httpHandler(newListEventDevicesHandler(c.Config))).Methods("GET")
	router.Handle("/cvds/{group}/{name}/event_devices/{device_name:[^/:]+}:inject",
		httpHandler(newInjectInputEventsHandler(c.Config, c.OperationManager))).Methods("POST")
	router.Handle("/cvds/{group}/{name}/event_devices/{device_name}/:inject",
		httpHandler(newInjectInputEventsHandler(c.Config, c.OperationManager))).Methods("POST")
...
type listEventDevicesHandler struct {
	Config Config
}

func newListEventDevicesHandler(c Config) *listEventDevicesHandler {
	return &listEventDevicesHandler{Config: c}
}

func (h *listEventDevicesHandler) Handle(r *http.Request) (interface{}, error) {
	vars := mux.Vars(r)
	group := vars["group"]
	name := vars["name"]
	opts := ListEventDevicesActionOpts{
		Selector:    cvd.InstanceSelector{GroupName: group, Name: name},
		ExecContext: exec.CommandContext,
	}
	return NewListEventDevicesAction(opts).Run()
}

type injectInputEventsHandler struct {
	Config Config
	OM     OperationManager
}

func newInjectInputEventsHandler(c Config, om OperationManager) *injectInputEventsHandler {
	return &injectInputEventsHandler{Config: c, OM: om}
}

func (h *injectInputEventsHandler) Handle(r *http.Request) (interface{}, error) {
	vars := mux.Vars(r)
	group := vars["group"]
	name := vars["name"]
	...
	opts := InjectInputEventsActionOpts{
		Selector:         cvd.InstanceSelector{GroupName: group, Name: name},
	...
	res, err := NewInjectInputEventsAction(opts).Run()
	if err != nil {
		os.Remove(tempFile)
		return nil, err
	}

...
type InjectInputEventsActionOpts struct {
...
type InjectInputEventsAction struct {
	selector       cvd.InstanceSelector
	deviceName     string
	eventsFilePath string
	om             OperationManager
...
func NewInjectInputEventsAction(opts InjectInputEventsActionOpts) *InjectInputEventsAction {
	return &InjectInputEventsAction{
...
func (a *InjectInputEventsAction) Run() (apiv1.Operation, error) {
	...
	go func(op apiv1.Operation, filePath string) {
		...
		if err := a.cvdCLI.LazySelectInstance(a.selector).InjectInputEvents(a.deviceName, filePath); err != nil {

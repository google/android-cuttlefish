...
func NewCvd(execCtx hoexec.ExecContext) *Cvd {
	return &Cvd{execCtx}
}

func (cvd *Cvd) buildCmd(bin string, args ...string) *exec.Cmd {
	return cvd.execContext(context.TODO(), bin, args...)
}

func (cvd *Cvd) runCmd(cmd *exec.Cmd) ([]byte, error) {
...
func (cvd *Cvd) exec(bin string, args ...string) ([]byte, error) {
	const begin = "############################################\n" +
		"## BEGIN \n" +
		"############################################"
	const end = "############################################\n" +
	...
	return cvd.runCmd(cvd.buildCmd(bin, args...))
}

func (cvd *Cvd) Load(configPath string, creds FetchCredentials) (*Group, error) {
	cmd := cvd.buildCmd(CVDBin, "load", configPath)
	if creds != nil {
		creds.AddToCmd(cmd)
	}

	out, err := cvd.runCmd(cmd)
	if err != nil {
...
func (cvd *Cvd) Create(selector Selector, createOpts CreateOptions, startOpts StartOptions) (*Group, error) {
	args := selector.asArgs()
	args = append(args, "create")
	args = append(args, createOpts.toArgs()...)
	args = append(args, startOpts.toArgs()...)
	out, err := cvd.exec(CVDBin, args...)
	if err != nil {
...
func (cvd *Cvd) Start(selector Selector, opts StartOptions) error {
	args := selector.asArgs()
	args = append(args, "start", "--report_anonymous_usage_stats=y")
	args = append(args, opts.toArgs()...)
	_, err := cvd.exec(CVDBin, args...)
	return err
}

func (cvd *Cvd) Stop(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) Remove(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) Fleet() (*Fleet, error) {
	out, err := cvd.exec(CVDBin, "fleet")
	if err != nil {
...
func (cvd *Cvd) BugReport(selector Selector, includeADBBugReport bool, dst string) error {
	args := selector.asArgs()
...
func (cvd *Cvd) Suspend(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) Resume(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) TakeSnapshot(selector Selector, dir string) error {
	args := selector.asArgs()
...
func (cvd *Cvd) PowerWash(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) PowerBtn(selector Selector) error {
	args := selector.asArgs()
...
func (cvd *Cvd) Fetch(opts FetchOpts, creds FetchCredentials, targetDir string) error {
	...
	cmd := cvd.buildCmd(FetchCVDBin, args...)
	if creds != nil {
		creds.AddToCmd(cmd)
	}
	if _, err := cvd.runCmd(cmd); err != nil {
	...
	if _, err := cvd.exec("chmod", "-R", "g+rw", targetDir); err != nil {
		return err
	}
	return nil
}

...
type execCvdCommand interface {
	exec(cvd *cvd.Cvd, sel cvd.Selector) error
}

type startCvdCommand struct{}

func (a *startCvdCommand) exec(cvdTool *cvd.Cvd, sel cvd.Selector) error {
	return cvdTool.Start(sel, cvd.StartOptions{})
}

type stopCvdCommand struct{}

func (a *stopCvdCommand) exec(cvd *cvd.Cvd, sel cvd.Selector) error {
	return cvd.Stop(sel)
}

type removeCvdCommand struct{}

func (a *removeCvdCommand) exec(cvd *cvd.Cvd, sel cvd.Selector) error {
	return cvd.Remove(sel)
}

type powerwashCvdCommand struct{}

func (a *powerwashCvdCommand) exec(cvd *cvd.Cvd, sel cvd.Selector) error {
	return cvd.PowerWash(sel)
}

type powerbtnCvdCommand struct{}

func (a *powerbtnCvdCommand) exec(cvd *cvd.Cvd, sel cvd.Selector) error {
...
func NewExecCVDCommandAction(opts ExecCVDCommandActionOpts) *ExecCVDCommandAction {
	return &ExecCVDCommandAction{
		...
		cvdTool:  cvd.NewCvd(exec.NewAsUserExecContext(opts.ExecContext, opts.CVDUser)),
	}
}
...
func (a *ExecCVDCommandAction) Run() (apiv1.Operation, error) {
	...
	go func(op apiv1.Operation) {
		...
		result.Error = a.command.exec(a.cvdTool, a.selector)

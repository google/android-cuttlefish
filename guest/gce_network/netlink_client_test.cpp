...
class NetlinkClientTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    // These calls should happen every time netlink client is created.
    EXPECT_CALL(sys_client_, Socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE))
        .WillOnce(Return(kNetlinkSocket));
    EXPECT_CALL(sys_client_, Socket(AF_UNIX, SOCK_DGRAM, 0))
        .WillOnce(Return(kNetworkSocket));
    // These calls should happen every time netlink client is destroyed.
    EXPECT_CALL(sys_client_, Close(kNetlinkSocket));
    EXPECT_CALL(sys_client_, Close(kNetworkSocket));
    nl_client_.reset(NetlinkClient::New(&sys_client_));
  }
 ...
 protected:
  ::testing::StrictMock<test::MockSysClient> sys_client_;

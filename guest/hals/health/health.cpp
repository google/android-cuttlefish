...
class HealthImpl : public Health {
 ...
 private:
  ...
  BatteryStatus getBatteryStatus(const char* path);
...
BatteryStatus HealthImpl::getBatteryStatus(const char* path) {
  auto status = getValue(path);
  if (status == "Charging") return BatteryStatus::CHARGING;
  if (status == "Discharging") return BatteryStatus::DISCHARGING;
  if (status == "Not charging") return BatteryStatus::NOT_CHARGING;
  if (status == "Full") return BatteryStatus::FULL;

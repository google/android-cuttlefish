...
void PopulateEglAndGlesAvailabilityImpl(GraphicsAvailability* availability) {
  auto egl = Egl::Load();
  if (!egl) {
    LOG(DEBUG) << "Failed to load EGL library.";
    return;
  }
  LOG(DEBUG) << "Loaded EGL library.";
  ...
  if (display != EGL_NO_DISPLAY) {
    ...
    LOG(DEBUG) << "Failed to get default display. " << egl->eglGetError()
  ...
  if (gles2_context == EGL_NO_CONTEXT) {
    ...
    if (egl->eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE,
      ...
      LOG(DEBUG) << "Failed to make GLES2 context current.";
      return;
    }
    LOG(DEBUG) << "Make GLES2 context current.";
    availability->can_init_gles2_on_egl_surfaceless = true;

    auto gles = Gles::LoadFromEgl(&*egl);
    if (!gles) {
      LOG(DEBUG) << "Failed to load GLES library.";
      return;
    }
  ...
  if (gles3_context == EGL_NO_CONTEXT) {
    LOG(DEBUG) << "Failed to create GLES3 context.";
  } else {
    LOG(DEBUG) << "Created GLES3 context.";
    Closer context_closer(
        [&]() { egl->eglDestroyContext(display, gles3_context); });

    if (egl->eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                            gles3_context) != EGL_TRUE) {
      LOG(DEBUG) << "Failed to make GLES3 context current.";
      return;
    }
    LOG(DEBUG) << "Make GLES3 context current.";
    ...
    if (gles3_vendor == nullptr) {
      LOG(DEBUG) << "Failed to query GLES3 vendor.";
      return;
    }
    const std::string gles3_vendor_string((const char*)gles3_vendor);
    LOG(DEBUG) << "Found GLES3 vendor: " << gles3_vendor_string;
    ...
    LOG(DEBUG) << "Found GLES3 version: " << gles3_version_string;
    ...
    if (gles3_renderer == nullptr) {
      LOG(DEBUG) << "Failed to query GLES3 renderer.";
      return;
    }
    const std::string gles3_renderer_string((const char*)gles3_renderer);
    LOG(DEBUG) << "Found GLES3 renderer: " << gles3_renderer_string;
    ...
    if (gles3_extensions == nullptr) {
      LOG(DEBUG) << "Failed to query GLES3 extensions.";
      return;
    }
    const std::string gles3_extensions_string((const char*)gles3_extensions);
    LOG(DEBUG) << "Found GLES3 extensions: " << gles3_extensions_string;

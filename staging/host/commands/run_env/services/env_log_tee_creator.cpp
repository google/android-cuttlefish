...
EnvLogTeeCreator::EnvLogTeeCreator(
    const CuttlefishConfig::EnvironmentSpecific& environment)
    : environment_(environment) {}

Command EnvLogTeeCreator::CreateLogTee(Command& cmd,
                                       const std::string& process_name) {
  auto name_with_ext = process_name + "_logs.fifo";
  auto logs_path = environment_.PerEnvironmentPath(name_with_ext.c_str());
  ...
  return Command(HostBinaryPath("log_tee"))
      ...
      .AddParameter("--env_name=", environment_.environment_name());
}
